...
checkValuesYAML(){
    ...
    if cmp -s .bob/var.compareToLine .bob/var.scrapedLine; then
        echo "SUCCESS: values.yaml containes all the lines necessary for metrics exposure.";
...
checkCoreApplicationJAVA(){
    ...
    echo -e "return restTemplateBuilder.setConnectTimeout(connectionTimeout).build()" >> .bob/var.compareToLine;
    ...
    find ./src/main/java -regex '.*Application.java' -exec grep -o 'return restTemplateBuilder.setConnectTimeout(connectionTimeout).build()' {} \; >> .bob/var.scrapedLine || true;
