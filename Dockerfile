...
RUN zypper ar -C -G -f $CBOS_REPO_URL?ssl_verify=no \
    ...
    && zypper install -l -y catatonit java-17-openjdk-headless \
...
ARG USER_NAME="eric-oss-4gpmevent-filetrans-proc"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-4gpmevent-filetrans-proc-app.jar
...
-Dcom.sun.management.jmxremote.access.file=/jmx/jmxremote.access -jar eric-oss-4gpmevent-filetrans-proc-app.jar
...
LABEL \
    org.opencontainers.image.title=eric-oss-4gpmevent-filetrans-proc-jsb \
